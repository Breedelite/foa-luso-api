...
def plotDetails(details,stufftoplot=['cost','profit','disease','newseedbank','weedpenalty','newN'],lus=None,lulist=None):
    ...
    if not (lus==None or lulist==None):
        xticks(range(len(lus)),[lulist[lu]['label'] for lu in lus])
...
def testall(ny,nlu,parameters,lulist,stochMultsUsed=None,optionalparams=None):
    ...
    while i < maxi:
        ...
        if bestp<p:
            ...
            bestlus=lus[:]
            print('new best LU')
            print([lulist[i]['name'] for i in lus], end=' ')
            print(bestp)
        i=i+1
...
def testBreak2(ny,nlu,parameters,lulist,mainlu,optionalparams=None):
    ...
    while cont:
        ...
        print("loop",loop)
        print('new best LU')
        print([lulist[i]['name'] for i in bestlus], end=' ')
        print(bestprofit)
        cont=False
        newbestlus=bestlus[:]
...
def randsearch(ny,nsols,parameters,lulist,maxstuck=10000,collectData=False,optionalparams=None):
    ...
    while stuck<maxstuck:
        ...
        if bestluslist[0][0]<p:
            ...
            print([lulist[i]['name'] for i in lus],p)
            if collectData:
                collectedData.append([p,perf_counter()-starttime])
    endtime=perf_counter()
    print('that took',endtime-starttime,'seconds')
    if collectData:
...
def GA(ny,parameters,lulist,maxgens=100,maxstuck=10,popsize=500,
    ...
    for gen in range(maxgens):
        ...
        for i in range(popsize-nsurvivors):
            ...
            pop.append(baby)
        if collectData:
            collectedData.append([bestp,perf_counter()-starttime])
    endtime=perf_counter()
